...
int equalStacks(vector<int> h1, vector<int> h2, vector<int> h3) {
    ...
    stack<int> a,b,c;
    for(int i=h1.size()-1;i>=0;i--) a.push(h1[i]);
    for(int i=h2.size()-1;i>=0;i--) b.push(h2[i]);
    for(int i=h3.size()-1;i>=0;i--) c.push(h3[i]);
    ...
    while(true){
        ...
        if(a.top()==b.top() && b.top()==c.top()){
            ...
            if(a.top()>=b.top() && a.top()>=c.top()) a.pop();
            else if(b.top() >= a.top() && b.top() >= c.top()) b.pop();
            else c.pop();
        }
    }
