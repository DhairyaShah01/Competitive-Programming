...
int twoStacks(int x, vector<int> a, vector<int> b) {
    ...
    while(i < a.size() && sum+a[i]<=x){ // considering only one stack and calculating count
        sum += a[i];
        i++;
    }
    count = i;
    //adding one element of second stack at a time and
    //subtracting top element of first stack if sum is greater than x
    while(j<b.size() && i>=0){
        sum += b[j];
        j++;
        while(sum>x && i>0){
